rMapskr = setRefClass('rMapskr', list(params = 'list', lib = 'character', 
  ...
  addControls = function(nm, value, values, label = paste("Select ", nm, ":")){
    .self$setTemplate(
      page = 'rMapskrControls2.html',
      script = system.file('libraries', lib, 'controls', 
        'script.html', package = 'rMapskr')
    )
    .self$set(width = 700)
  ...
  show = function(mode_ = NULL, ..., extra_files = NULL){
    ...
    switch(mode_, 
       static = {
         dir.create(temp_dir <- tempfile(pattern = 'rMapskr'))
      ...
      server = {
        ...
        shiny::runApp(
          file.path(system.file(package = "rMapskr"), "shiny"),
